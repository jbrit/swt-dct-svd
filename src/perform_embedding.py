...
def integrate_embedding():
    ...
    for file in os.listdir(config.train_images_path):
        ...
        i = int(img.shape[0] // 2) * 2
        j = int(img.shape[1] //  2) * 2
        img = img[:i,:j]
        # img = np.asarray(img.resize((256, 256)))

        LL, (LH, HL, HH) = dwt_2d(img, 'haar') # get the coefficients of DWT transform
        print(LL.shape)
        coeffs = pywt.swtn(img, 'haar', 1)[0]
        LL, LH, HL, HH = coeffs['aa'], coeffs['ad'], coeffs['da'], coeffs['dd']
        ...
        embedded_image = pywt.iswtn(coeffs=[{**coeffs, "aa": iLL }], wavelet='haar')
